    ...
    try:
        while True:
            ...
            if ISS.at(t).is_sunlit(ephemeris):
                while (datetime.datetime.now() < start_time + timedelta(seconds=25)):
                    photo.get_photo(camera)
    ...
    try:
        ...
        while (datetime.datetime.now() < start_time + timedelta(minutes=30)):
            ...
            if shadow.sun_data.altitude(coordinates_latitude=latitude, coordinates_longtitude=longitude, year=year, month=month, day=day, hour=hour, minute=minute, second=second) > 5:
                ...
                north_main_fast = north.find_north_fast(image_1=image_1_path, image_2=image_2_path)
                print("sever",north_main)
                print("sever fast",north_main_fast)
                ...
                for images in os.listdir("./chop/"):
                    ...
                    if 50 < properties.calculate_brightness(chop_image_path) < 190 and properties.calculate_contrast(chop_image_path) > 42:
                        ...
                        print("Skipped image due to the brightness being", properties.calculate_brightness(chop_image_path), "or the image was so noncontrast", properties.calculate_contrast(chop_image_path))
                    print("")
                    print("NEW SECTOR", sector_id+1)
                    print("")
                    sector_id += 1
                else:
            ...
            print("")
            print("......")
            print("NEW SECTOR", full_image_id+1)
            print("......")
            print("")
