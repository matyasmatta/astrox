    ...
    def run(self):
        ...
        while (datetime.now() < start_time + timedelta(minutes = 175)): 
            if directory.get_size() < 3000000000: # cca. 2.8 gigabytes
                ...
                t = timescale.now()
                print("photo_multiplier", photo_multiplier)
                    ...
                    def accuracy():
                        ...
                        if len(store_edoov_coefficient) > 1:
                            if abs(all_edoov_coefficient - list_medianu) > 3:
                                print(all_edoov_coefficient, list_medianu)
                                eda_bad_accuracy = True
                        print(eda_bad_accuracy)
                        return eda_bad_accuracy
                    eda_bad_accuracy = True
                    while (datetime.now() < start_time + timedelta(seconds=30)) or eda_bad_accuracy:
                        print("Conditions for while loop were met")
                        print("img total", count_for_images_day)
                        print("eda cunt", eda_count)
                        global all_edoov_coefficient
                        if eda_count+1 < count_for_images_day:
                            try:
                                print("Conditions for calculating were met")
                                i_1=str(eda_count)
                                before = "main/img_"
                            ...
                            print("neni obrayek asi", eda_count)
                            print("I'm sleeping")
                            sleep(photo_sleep_interval) # nefunguje si mysli eda
                    print("vypocteno")
                    global photo_multiplier
                    photo_multiplier = 1
                    print("photo_multiplier v mem kodu", photo_multiplier)
                except:
