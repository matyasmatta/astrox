...
class photo:
    def get_photo(camera):
        ...
        camera.capture("./zkouska_koeficientu/photed" + imageName)
    def convert(angle):
...
try:
    ...
    camera = PiCamera()
    sleep(2)
    count_for_edovo_srac = 0
    while (datetime.now() < start_time + timedelta(seconds=256)):
        photo.get_photo(camera)
        i_1=str(count_for_edovo_srac)
        before = "./zkouska_koeficientu/uploaded/img_"
        image_1=str(before + i_1 +".jpg")
        i_2=str(count_for_edovo_srac+1)
        #print(image_1)
