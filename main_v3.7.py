...
class split:
    def file_split(image_id, image_path, poloha_severu):
        # check if the image ends with png or jpg or jpeg
        if (image_path.endswith(".png") or image_path.endswith(".jpg") or image_path.endswith(".jpeg")):
            # Opens an image in RGB mode
            #rotates photo so the north is always on top
            image = Image.open(image_path)
            
            rotated_image = image.rotate(poloha_severu)
            rotated_image.show()
            ...
            for x0 in range(0, width, chopsize):
                for y0 in range(0, height, chopsize):
                    ...
                    file_name = "./chop/astrochop_" + str(within_loop_counter + "_" + y0 + "_" + x0) + ".jpg" #nove - potrebuje otestovat
